...
class MsgpackRpcClient:
    def __init__(self, ip="127.0.0.1", port=41451, timeout=60):
...
class VehicleClient:
    def __init__(self, ip = "", port = 41451, timeout_value = 3600):
        if ip == "":
            ip = "127.0.0.1"
        self.client = MsgpackRpcClient(ip, port, timeout=timeout_value)
...
class MultirotorClient(VehicleClient):
    ...
    def takeoffAsync(self, timeout_sec = 20, vehicle_name = ''):
        ...
        Returns:
            concurrent.futures.Future: future. call .result() to wait for method to finish. Example: client.METHOD().result()
        """
